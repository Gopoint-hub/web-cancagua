# Estructura del Menú Cancagua

## SERVICES (Servicios)
- Geothermal Biopools → /servicios/biopiscinas
- Hot Tubs → /servicios/hot-tubs
- Sauna Nativo → /servicios/sauna
- Massages & Therapies → /masajes
- Regular Classes → /clases

## EXPERIENCIAS
- Navega Relax → /experiencias/navega-relax
- Pases Reconecta → /experiencias/pases-reconecta
- Full Day Hot Tubs + Biopiscinas → /experiencias/full-day-hot-tubs-biopiscinas

## EVENTS
- Todos los Eventos → /eventos
- Heart Coherence Workshop → /eventos/heart-coherence-workshop
- Taller Wim Hof → /eventos/taller-wim-hof
- Eventos Sociales → /eventos/sociales
- Eventos Empresas → /eventos/empresas
