  ...
  return <>
    ...
    <aside role="dialog" aria-modal="true" aria-label="Carrito de servicios" className={`fixed inset-y-0 right-0 z-[80] flex w-full max-w-lg flex-col bg-[#F8F6F1] shadow-2xl transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}>
      ...
      <form onSubmit={pay} className="flex min-h-0 flex-1 flex-col"><div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        {suggestion && <a href={suggestion.href} className="group block rounded-[1.5rem] border border-[#899169] bg-[#D8DACD] p-5 text-center shadow-[0_4px_6px_rgba(0,0,0,0.04),0_10px_18px_rgba(0,0,0,0.05)] transition duration-300 hover:-translate-y-0.5 hover:bg-[#CFD2C3]"><span className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-[#4A4F35] text-[#FCF9F9]"><Sparkles className="h-5 w-5" /></span><span className="mt-3 block font-cg-mono text-[10px] uppercase tracking-[0.18em] text-[#696F4D]">Completa tu experiencia</span><strong className="mt-1 block font-cg-serif text-xl font-light text-[#282C1C]">{suggestion.label} →</strong></a>}
