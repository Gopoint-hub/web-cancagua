...
export default function ServicePaymentResult() {
  ...
  if (result.status === "paid") return <ResultShell icon={<CheckCircle2 className="h-14 w-14 text-green-700" />} title="¡Tus reservas están confirmadas!" description={`Enviamos los comprobantes y detalles de cada servicio a ${result.clientEmail}.`}>
    <div className="mt-7 space-y-3 text-left">{result.items.map(item => {
      const label = item.module === "biopools" ? "Biopiscinas" : item.module === "sauna" ? "Sauna" : item.module === "massages" ? "Masaje" : "Clases Regulares";
      const detail = item.module === "regular_classes" ? `Plan activo desde ${String(item.bookingDate).slice(0, 10)}` : `${String(item.bookingDate).slice(0, 10)} · ${item.startTime}–${item.endTime} · ${item.guests} persona${item.guests === 1 ? "" : "s"}`;
      return <article key={item.id} className="rounded-2xl bg-[#F4F2ED] p-5"><p className="font-cg-mono text-[10px] uppercase tracking-[0.16em] text-[#4B5872]">{label}</p><div className="mt-2 flex flex-col justify-between gap-2 sm:flex-row"><div><h2 className="font-cg-serif text-2xl">{item.itemName}</h2><p className="mt-1 font-cg-soft text-sm text-[#635E5A]">{detail}</p></div><div className="sm:text-right"><p className="font-cg-soft text-xs uppercase text-[#827D78]">Estado</p><strong className="font-cg-mono text-sm">{item.bookingCode || "Confirmada"}</strong></div></div></article>;
    })}</div>
