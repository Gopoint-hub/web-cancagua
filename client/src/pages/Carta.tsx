...
export default function Carta() {
    ...
    if (prices.for_2 || prices.for_4 || prices.for_6) {
      return (
        <div className="space-y-1">
          {prices.for_2 && <p className="text-sm text-gray-700"><T>Para 2</T>: {formatPrice(prices.for_2)}</p>}
          {prices.for_4 && <p className="text-sm text-gray-700"><T>Para 4</T>: {formatPrice(prices.for_4)}</p>}
          {prices.for_6 && <p className="text-sm text-gray-700"><T>Para 6</T>: {formatPrice(prices.for_6)}</p>}
        </div>
      );
    }
    if (prices.default) {
  ...
  return (
    <AutoTranslateProvider pageId="carta">
          ...
          <section className="relative bg-[#4A4F35] text-white py-20">
            <div className="container mx-auto px-4 text-center">
              ...
              <p className="text-lg max-w-2xl mx-auto">
                ...
                <br />
                <T>Siempre incluyendo opciones veganas y sin gluten.</T>
              </p>
              <p className="text-sm mt-4 opacity-80">
                <T>SUJETO A CAMBIOS SEGÚN DISPONIBILIDAD DE PRODUCTOS</T>
              </p>
            </div>
          </section>
            ...
            <div className="container mx-auto px-4 max-w-4xl">
              ...
              {!isLoading && menuData && menuData.length === 0 && (
                <div className="text-center py-20">
                  <p className="text-gray-600"><T>La carta estará disponible próximamente.</T></p>
                </div>
              )}
                ...
                return (
                  <div key={category.id} className="mb-16">
                    <div className="text-center mb-8">
                      <h2 className="text-3xl font-serif text-[#4A4F35] mb-2">
                        <T>{category.name}</T>
                      </h2>
                          ...
                          <div className="flex flex-col md:flex-row gap-4">
                            ...
                            <div className="flex-1">
                              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2 mb-3">
                                <h3 className="text-xl font-semibold text-gray-900"><T>{item.name}</T></h3>
                                {renderPrices(item.prices)}
                              </div>
                              {item.description && (
                                <p className="text-gray-600 mb-3 leading-relaxed"><T>{item.description}</T></p>
                              )}
                                ...
                                <div className="flex flex-wrap gap-2 mb-3">
                                  {item.dietaryTags.map((tag: string) => (
                                    <Badge key={tag} variant="outline" className="bg-sage-100 text-sage-700 border-sage-300">
                                      {dietaryIcons[tag]}
                                      <span className="ml-1"><T>{dietaryLabels[tag]}</T></span>
                                    </Badge>
                                  ))}
                                </div>
                              )}
